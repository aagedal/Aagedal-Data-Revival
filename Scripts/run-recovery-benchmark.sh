...
for fixture in manifest["fixtures"]:
    ...
    expected = [originals[item_id]["sha256"] for item_id in fixture["expectedOriginalIDs"]]
    exact = sum(digest in recovered_hashes for digest in expected)
    ...
    exact_total += exact
    fixture_results.append((fixture["id"], exact, len(expected), len(recovered_hashes)))
...
for identifier, exact, expected, recovered_count in fixture_results:
    print(f"{identifier}: {exact}/{expected} byte-exact originals; "
          f"{recovered_count} distinct recovered payloads")
