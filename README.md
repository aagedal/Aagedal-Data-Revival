...
Status: runnable SwiftUI prototype with experimental PhotoRec-backed JPEG and common camera RAW scans for raw disk images. Physical-card imaging is connected through an authenticated, app-embedded privileged helper with source revalidation, resumable ddrescue progress, cancellation, and safe remount/eject controls. Production recovery engines and their corresponding sources are packaged for Release builds. Clean-machine and real-card acceptance testing is still required before release. Video recovery and claims of full camera RAW integrity are explicitly outside the 1.0 scope.
...
The current ddrescue adapter builds a shell-free, mapfile-backed imaging command, appends separate process output, and writes source-bound resume metadata before launch. Physical-card imaging uses the app-embedded privileged helper and the tested unmount/revalidation lifecycle; administrator approval is required before the helper can run.
