...
- **Initial recovery engine:** PhotoRec behind a process adapter. It has documented scripted operation and can scan disk images. Its progress reporting, cancellation, and resume behavior are isolated behind the integration boundary.
- **Card imaging:** GNU ddrescue behind a separate adapter, preserving its mapfile for interrupted or difficult reads.
- **Device access:** Disk Arbitration for discovery and lifecycle management; macOS `authopen` for temporary, path-specific read-only raw-device access. Keep ddrescue and the UI unprivileged.
...
The current ddrescue adapter builds a shell-free, mapfile-backed imaging command, appends separate process output, and writes source-bound resume metadata before launch. Physical-card imaging uses a read-only descriptor returned by macOS `authopen` and the tested unmount/revalidation lifecycle; administrator authorization is requested when imaging starts, without installing a background service.
...
1. Native UI foundation with explicitly labeled sample data and the source-to-results workflow.
